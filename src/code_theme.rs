...
/// The Tokyo Night theme from https://github.com/enkia/enki-theme/blob/master/scheme/Enki-Tokyo-Night.tmTheme
const ENKI_TOKYO_NIGHT_CONFIG: &[u8; 44344] = br#"<?xml version="1.0" encoding="UTF-8"?>
...
// not safe, but works here as it only handles this specific case.
pub struct UnicodeBuffer {
    pub data: &'static [u8],
    pub pos: u64,
}

impl std::io::Read for UnicodeBuffer {
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        let buf_len = buf.len();
        let data_len = self.data.len();
        let pos = self.pos as usize;
        let remaining = data_len - pos;
        let to_read = std::cmp::min(buf_len, remaining);
        buf[..to_read].clone_from_slice(&self.data[pos..pos + to_read]);
        self.pos += to_read as u64;
        Ok(to_read)
    }
}

impl std::io::BufRead for UnicodeBuffer {
...
impl std::io::Seek for UnicodeBuffer {
    fn seek(&mut self, pos: std::io::SeekFrom) -> std::io::Result<u64> {
        let new_pos = match pos {
            std::io::SeekFrom::Start(n) => n,
            std::io::SeekFrom::End(n) => (self.data.len() as i64 + n) as u64,
...
/// The Tokyo Night theme from https://github.com/enkia/enki-theme/blob/master/scheme/Enki-Tokyo-Night.tmTheme
pub static ENKI_TOKYO_NIGHT_THEME: LazyLock<Theme> = LazyLock::new(|| {
    let mut reader = UnicodeBuffer {
        data: ENKI_TOKYO_NIGHT_CONFIG,
        pos: 0,
    };
